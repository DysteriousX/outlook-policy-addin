...
exports.extractPolicyIdFromFilename = extractPolicyIdFromFilename;
...
function extractPolicyIdFromFilename(filename) {
    const patterns = [
        /POL(\d+)/i,
        /Policy[-_](\d+)/i,
        /(\d{5,})/,
...
class PasswordRequiredError extends Error {
    constructor(attachmentName, zipEntryName) {
        super(`Attachment "${attachmentName}"${zipEntryName ? ` (file: ${zipEntryName})` : ""} is password-protected and requires a password.`);
 ...
 */
async function validateSinglePolicy(id, name, policyId, normalisedRecipients, client) {
...
async function validateAttachment(attachment, normalisedRecipients, client, password) {
            ...
            for (const entry of zipEntries) {
                if (!entry.isDirectory) {
                    ...
                    if (isPdf || entryPolicyId) {
                        ...
                        const entryResult = await validateSinglePolicy(`${attachment.id}/${entry.name}`, `${attachment.name} / ${entry.name}`, entryPolicyId, normalisedRecipients, client);
    ...
    const result = await validateSinglePolicy(attachment.id, attachment.name, singlePolicyId, normalisedRecipients, client);
    return [result];
}
...
async function validate(request, client, password) {
    ...
    const attachmentResultGroups = await Promise.all(request.attachments.map((att) => validateAttachment(att, normalisedRecipients, client, password)));
